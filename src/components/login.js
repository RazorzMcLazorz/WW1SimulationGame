...
class Login extends Component {
  ...
  getProducts = _ => {
    fetch('https://ww1simdb.herokuapp.com/sql')
  ...
  renderProduct = ({user_id, user_fname, user_lname, user_password, user_code}) => <div key={user_id}>{`${user_id} ${user_fname} ${user_lname} ${user_password} ${user_code}`}</div>

  login() { 
    const mysql = require("mysql");
    // First you need to create a connection to the db

    console.log(document.getElementById('fname').value + ' : ' + document.getElementById('pass').value);
  ...
  render() {
    return (
      ...
      <form>
        ...
        <Link to="/login">
          <input type="submit" value="submit" onClick={() => this.addProduct()}/>
        </Link>
      </form>
